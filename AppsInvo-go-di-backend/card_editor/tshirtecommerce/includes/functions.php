...
class dg{
	...
	public function saveDesign()
	{
		...
		$designer_id				= 'c9b7dacd4b016232934a50fbb4c28b4a';
		...
		else
		{
			...
			$content = array(
				...
				'back_background_color'=> $data['back_background_color'],
				'category'=> $data['category'],
				'design_name'=> $data['design_name'],
				'card_type'=> $data['card_type'],
				'price'=> $data['price'],
				'deign_edit_id'=> $data['deign_edit_id'],	
				'data_json'=> $design['vectors'],					
			);	
			//print_r($content);die;		 		
			$results['content'] = $content;	
			$json=json_encode($design[vectors]);
...
//print_r($content);die;
		...
		$con = new mysqli("localhost", "go_di", "godi@123", "go_di");
			...
			if($data['deign_edit_id'] != ''){


$mysqli_query = mysqli_query($con,"update template set template_name='$data[design_name]',
					canvas_thumbnail='$file',
					canvas_json=$json,
					...
					design_id='$key',
					designer_id='$user'
					WHERE template_id='$data[deign_edit_id]'");
			}else{
$mysqli_query = mysqli_query($con,"INSERT INTO template (template_name, canvas_thumbnail, canvas_json,cat_id,price,background_color,card_type,back_background_color,design_id,designer_id)
				VALUES ('$data[design_name]', '$file', $json,'$data[category]','$data[price]','$data[background_color]','$data[card_type]','$data[back_background_color]','$key','$user')");
			}
